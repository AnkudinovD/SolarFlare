...
public class MainActivity extends AppCompatActivity {
    ...
    private int[] delayFlash = {60, 70, 85}; // продолжительность вспышки на солнце на Марсе, Юпитере, Сатурне (в часах)
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        output.setText(coreTime(delayMinute, volumeData, coreFrequency, delayFlash) + " секунд");
    }
    ...
    private int volumeTime(int coreFrequency, int volumeData, int delayFlash) {
        return ((volumeData * delayFlash)/coreFrequency)/60;
    }
    ...
    private int coreTime(int[] delayMinute, int[] volumeData, int frequency, int[] solarDelayFlash) {
        ...
        for (int i = 0; i < volume.length; i++) { // инициализация данного массива
            volume[i] = volumeTime(frequency, volumeData[i], solarDelayFlash[i]); // определение времени обработки компьютером данных от одного спутника
        }
